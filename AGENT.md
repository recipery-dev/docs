...
This site documents [Recipery](https://github.com/recipery-dev/recipery), a self-hosted recipe manager — this repo (`recipery-dev/docs`) only holds the documentation, not the application source. For anything not covered here — implementation details, source code, open issues, exact env var handling — fetch the app repo directly rather than guessing.
