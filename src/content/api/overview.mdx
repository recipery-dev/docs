...
| Level | Applies to |
| --- | --- |
| Anyone with a valid session | Reading recipes, collections, settings; updating your own rating/favorite/cooked state; managing your own shopping list |
| The profile itself, or an admin | Editing a profile's name, color, or password |
| Admin only | Creating/deleting profiles, deleting recipes, changing roles, changing server-wide settings (library pagination, search limit, upload size), exporting a full backup |
...
| Shopping List — read, add/remove recipes, check off items | [Shopping List](/api/shopping-list) |
| Settings — library pagination, search limit, upload size, data export | [Settings](/api/settings) |
