...
| `sourceUrl` | string | No | |
| `videoUrl` | string | No | Must be http/https; a YouTube link embeds in the recipe view, anything else shows as a "Watch the video" link |
...
for how the scraper works. A YouTube URL is handled separately: it
always sets `videoUrl`, `title`, `source` (channel name), and the cover
photo from YouTube's oEmbed endpoint, and best-effort parses ingredients
and steps out of the video description when it finds any.
...
(YouTube URLs only 422 when the video itself can't be found).
